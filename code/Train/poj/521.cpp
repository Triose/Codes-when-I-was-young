...
LL powerfastmod(LL a, LL b, LL mod, LL n) {
    LL n1 = n;
    while(n1) {
	LL ta = a, tb = b;
	if(n1 & 1) {
	    a = (7 * ta % mod + 12 * tb % mod) % mod;
	    b = (4 * ta % mod + 7 * tb % mod) % mod;
	}
	ta = a; tb = b;
	a = (ta * ta % mod + 3 * tb * tb % mod) % mod;
	b = (2 * ta * tb) % mod;
	n1 >>= 1;
    }
    return 2 * a % mod + (n % 2 ? -1 : 1);
}
...
//  freopen("Out.txt", "w", stdout);
    ...
    while(~sfI(n)) {
	if(n == 1) pf(13);
	else pfI(powerfastmod(7LL, 4LL, MOD, n));
    }
    return 0;
}
